...
impl Item {
    ...
    pub async fn set_menu(&mut self, widget: &gtk::Box) -> zbus::Result<()> {
        let menu = DBusMenu::new(self.sni.inner().destination(), &self.sni.menu().await?);
        menu.set_parent(widget);
        self.gtk_menu = Some(menu);
        Ok(())
    }

    pub async fn popup_menu(&self, x: i32, y: i32) -> zbus::Result<()> {
    ...
    pub async fn icon(&self, image: &Image, size: i32, scale: i32) -> Option<Paintable> {
        // TODO explain what size and scale mean here

        // see icon.rs
        load_icon_from_sni(&self.sni, image, size, scale).await
    }
}
