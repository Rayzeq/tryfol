...
async fn fallback_icon(size: i32, scale: i32) -> Paintable {
...
pub async fn load_icon_from_sni(
    ...
    let icon_from_name: std::result::Result<Option<Paintable>, IconError> = (async {
        // fetch icon name
        ...
        if icon_path.is_absolute() && icon_path.is_file() {
            return gtk::gdk_pixbuf::Pixbuf::from_file_at_size(icon_path, scaled_size, scaled_size)
                .map(|p| Some(Texture::for_pixbuf(&p).upcast()))
    ...
    match icon_from_pixmaps {
        Ok(p) => return Some(Texture::for_pixbuf(&p).upcast()),
        Err(IconError::NotAvailable) => {}
        Err(e) => log::warn!(
    ...
    Some(fallback_icon(size, scale).await)
}
